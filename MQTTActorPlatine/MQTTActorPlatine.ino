...
volatile int lastStatus = LOW;
volatile bool stripOn = false;
double currentBrightness = 0.0;
volatile double redColorPart = 1.0;
volatile double greenColorPart = 0.0;
volatile double blueColorPart = 0.0;

void setup() {
  Serial.begin(9600);
  pinMode(RELAIS_PIN , OUTPUT);
  pinMode(LED_STRIP_RED_PIN, OUTPUT);
  pinMode(LED_STRIP_GREEN_PIN, OUTPUT);
  pinMode(LED_STRIP_BLUE_PIN, OUTPUT);
  
  setupWifiConnection(WIFI_SSID, WIFI_PASSWORD);

  relaisSwitch -> setupSubscription(mqttClient);
  rgbLedStrip -> setupSubscription(mqttClient);
